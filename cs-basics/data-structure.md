...
- Tree의 한 단계를 내려갈 수록 **depth(height)**가 1씩 올라갑니다.
...
예시로는 [AVL](https://en.wikipedia.org/wiki/AVL_tree), R-B Tree등이 있습니다. 
...
DB와 파일 시스템 등에 많이 쓰이는 구조입니다.

**Search, Insert, Delete 모두 `O(log n)`입니다.**

### R-B(Red-Black) Tree

<img src="./images/Red-black_tree_example_with_sockets.svg.png" width="500" />

**[Red-Black Tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree)는 self-balancing binary tree**로 정보의 저장과 retrieval(가져오기)이 빠릅니다. 각 Node는 빨강/검정의 속성을 추가로 가지고 있으며 이를 이용해 tree가 적절히 균형 잡힐 수 있도록 합니다(한 쪽 depth가 너무 깊거나 하지 않음)
