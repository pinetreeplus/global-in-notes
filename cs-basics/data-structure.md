...
**[Red-Black Tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree)는 self-balancing binary tree**로 정보의 저장과 retrieval(가져오기)이 빠릅니다. 각 Node는 빨강/검정의 속성을 추가로 가지고 있으며 이를 이용해 tree가 적절히 균형 잡힐 수 있도록 합니다. Binary Search Tree의 성질에 더해 다음이 충족되어야 R-B Tree입니다:
...
Worst case `O(log n)`을 보장하므로 다른 Worst Case 보장이 필요한 대이터 structure나 실시간 앱등에 쓰입니다.
...
**[AVL Tree](https://en.wikipedia.org/wiki/AVL_tree)는 leaf Node간의 depth가 1 이상 차이 나지 않는 Self-balancing binary search tree**입니다. 만약 2 이상 차이가 나게 된다면, tree는 rebalancing 되게 됩니다. R-B Tree와 자주 비교됩니다.

**AVL Tree는 Search, Insertion, Deletion 모두 `O(log n)`입니다.**'

AVL 역시 Red/Black으로 Node를 칠할 수 있으며 이 경우 R-B Tree의 일종으로 취급되곤 합니다.
