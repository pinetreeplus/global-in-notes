...
<img src="./html/element-anatomy.png" width="500">
...
<img src="./images/element-attributes.png" width="500">
...
[줄바꿈 예문](./html/line-breaks.html)은 다음과 같이 사용됩니다.:
...
[div-span 예문](./html/div-span.html) 은 semantic하지 않은 wrapper입니다. 다른 태그가 사용 가능하면 그걸 사용하는게 좋습니다.
...
시간은 [`<time>`](./html/time.html)이며 다양한 `datetime` 형식 사용 가능.
...
[`<table>`](./html/table.html)은 테이블 데이터를 표현.
...
[`a`](./html/links.html)는 `href` attribute와 함께 웹페이지, 파일, 이메일 주소, 같은 페이지의 다른 부분 등으로의 하이퍼 링크를 생성합니다.
...
[`<img>`](./html/image.html) 태그는 document에 이미지를 embed 합니다. 이는 **void element**로 자식 컨텐츠를 가질 수 없습니다(closing tag 없음). 제대로 사용하기 위해서는 `src`와 `alt` attribute이 필요합니다.
...
[`<video>`](./html/video.html) Element는 비디오를 embed 가능하게 해줍니다:
...
[`<audio>`](./html/audio.html) 태그는 음성 리소스를 embed 합니다. 하나 또는 그 이상의 오디오 소스를 embed 가능하며, `src` Attribute이나 `<source>` Element(브라우저가 가장 적합한 걸 고름)로 표현.

`<audio>` Element는 `width`/`height`와`poster` Attributes이 없다는 거 외에는 `<video>`와 동일

## form

[`<form>`](./html/form.html) 태크는 정보 제출을 위한 상호작용 가능한 섹션입니다.
...
[`<textarea>`](./html/textarea.html)는 여러줄 짜리 텍스트 입력창을 생셩하며 `<input>`과 별개의 Element입니다.

### select

[Click me](./html/select.html)
